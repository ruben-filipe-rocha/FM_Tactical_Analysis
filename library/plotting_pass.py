...
def criar_animacao_plotly_full(df, ranked_timestamps, equipa_azul, equipa_vermelha, equipa_esquerda_nome=None):
    ...
    for _, r in ranked_timestamps.iterrows():
       ...
       # ==========================
        ...
        colunas_base = ["equipa_jogador", "timestamp"]
        ...
        def extrair_metricas(equipa_nome, ts):
            linha = metricas_frame[
                (metricas_frame["equipa_jogador"] == equipa_nome) &
            ...
            return {
            "Stretch Index (yd)": f(l.get("stretch_index")),
            "Compactness (yd²)": f(l.get("compactacao")),
            "Team Balance (%)": f(l.get("indice_equilibrio_defensivo")),
            "Width (yd)": f(l.get("distancia_media_entre_jogadores")),
            "Team Length (yd)": f(l.get("distancia_coletiva_entre_equipas")),
            "Centroid Distance (yd)": f(l.get("team_centroid_distance")),
            "Surface Area (yd²)": f(l.get("espaco_criado")),
        }
    ...
    title=None,  # remove o título superior
    sliders=sliders,
    ...
    # ==========================
    fig.add_annotation(
    x=0.5, y=1.12,  # ↓ baixado para o mesmo nível dos títulos anteriores
    xref="paper", yref="paper",
            text=f"<b>{equipa_azul}</b> vs <b>{equipa_vermelha}</b>",
            showarrow=False,
            font=dict(color="black", size=20),
