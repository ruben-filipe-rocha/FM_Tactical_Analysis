...
# ==========================
def criar_animacao_plotly_full(df, ranked_timestamps, equipa_azul, equipa_vermelha, equipa_esquerda_nome=None):
    if equipa_esquerda_nome is None:
        esquerda, _ = obter_lado_equipa(df)
    ...
    for _, r in ranked_timestamps.iterrows():
        ...
        frame_data = df[(df["timestamp"] == ts) & (df["period"] == period)]
        if frame_data.empty:
            continue
       ...
       # ==========================
        ...
        if nome:
            ...
            legenda_equipa = dict(
                x=0.22, y=-0.09,  # posição superior (ligeiramente acima da segunda)
            ...
            legenda_jogador = dict(
                x=0.22, y=-0.16,  # posição ligeiramente mais abaixo
                xref="paper", yref="paper",
        ...
        # ==========================
        # 5.2.2 Título e métricas (todas as métricas do artigo)
        # ==========================
        import pandas as pd
        ...
        titulo_texto = f"Match Period: {period_text}"


        # colunas reais existentes no df
        metricas_cols = [
            "team_corrigido",
            "stretch_index",
            "compactacao",
            "indice_equilibrio_defensivo",
            "distancia_media_entre_jogadores",
            "indice_equilibrio_defensivo",
            "distancia_coletiva_entre_equipas",
            "espaco_criado"
        ]

        colunas_base = ["team_corrigido", "timestamp"]
        metricas_existentes = [c for c in metricas_cols if c in df.columns]
        metricas_frame = (
            df
            .groupby(colunas_base, as_index=False)
            .agg({
                "stretch_index": "mean",
                "compactacao": "mean",
                "indice_equilibrio_defensivo":"mean",
                "distancia_media_entre_jogadores": "mean",
                "indice_equilibrio_defensivo": "mean",
                "distancia_coletiva_entre_equipas": "mean",
                "espaco_criado": "mean"
            })
        )

        def extrair_metricas(equipa_nome, ts):
            linha = metricas_frame[
                (metricas_frame["team_corrigido"] == equipa_nome) &
                (metricas_frame["timestamp"] == ts)
            ]
            if linha.empty:
                return {}
            l = linha.iloc[0]
            ...
            return {
            "Team Expansion - Stretch Index (yd)": f(l.get("stretch_index")),
            "Team Compactness (yd²)": f(l.get("compactacao")),
            "Team Balance (%)": f(l.get("indice_equilibrio_defensivo")),
            "Lateral coverage - Width (yd)": f(l.get("distancia_media_entre_jogadores")),
            "Vertical Spacing - Team Length (yd)": f(l.get("distancia_coletiva_entre_equipas")),
            "Block Separation - Centroid Distance (yd)": f(l.get("team_centroid_distance")),
            "Space Creation - Surface Area (yd²)": f(l.get("espaco_criado")),
        }

        met_azul = extrair_metricas(equipa_azul, ts)
        met_vermelha = extrair_metricas(equipa_vermelha, ts)
        # Anotações das métricas no topo
        annotations_metricas = []
        y_base = 0.90
        y_step = -0.07

        annotations_metricas.append(dict(
            x=0.5, y=1.07,
            xref="paper", yref="paper",
            text=titulo_texto,
            showarrow=False,
        ...
        # ==========================
        # Título das métricas (por equipa)
        # ==========================
        annotations_metricas.append(dict(
            x=-0.01, y=y_base + 0.05,
            xref="paper", yref="paper",
            text=f"Metrics – {equipa_azul}",
            showarrow=False,
        ...
        annotations_metricas.append(dict(
            x=0.78, y=y_base + 0.05,
            xref="paper", yref="paper",
            text=f"Metrics – {equipa_vermelha}",
            showarrow=False,
        ...
        # Métricas por equipa
        # ==========================
        for i, (nome_metrica, valor) in enumerate(met_azul.items()):
            annotations_metricas.append(dict(
                x=-0.01, y=y_base + y_step * i,
                xref="paper", yref="paper",
                text=f"{nome_metrica}: {valor}",
                showarrow=False,
                font=dict(color="black", size=12),
                align="left"
            ))

        for i, (nome_metrica, valor) in enumerate(met_vermelha.items()):
            annotations_metricas.append(dict(
                x=0.78, y=y_base + y_step * i,
                xref="paper", yref="paper",
                text=f"{nome_metrica}: {valor}",
                showarrow=False,
                font=dict(color="black", size=12),
    ...
    fig.add_annotation(
        ...
        text=f"<b>{equipa_azul}</b> vs <b>{equipa_vermelha}</b>",
        showarrow=False, font=dict(color="black", size=20), align="center"
    )

    # ==========================
    # 5.7 Devolver a figura (sem fig.show)
    # ==========================
    fig.layout.pop("width", None)   # remove se tiver ficado definido
    fig.layout.pop("height", None)  # remove se tiver ficado definido
